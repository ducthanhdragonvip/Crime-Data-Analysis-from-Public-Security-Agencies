...
def ingest_crimes():
    ...
    data_fetcher = SocrataDataFetcher(
        domain=settings.SEATTLE_SOCRATA_DOMAIN,
        dataset_id=settings.SEATTLE_DATASET_ID,
        timestamp_file=settings.SEATTLE_TIMESTAMP_FILE,
        update_field=settings.SEATTLE_UPDATE_FIELD
    )
    ...
    try:
        ...
        for record in records:
            record_id = record.get('report_number')  # Seattle uses report_number instead of id
            if record_id:
                producer.send_message(settings.SEATTLE_KAFKA_TOPIC, record_id, record)
...
def main():
    ...
    logger.info("Starting Seattle Crimes Kafka ingestion service.")
    while True:
        try:
            ingest_crimes()
